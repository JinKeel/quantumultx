  ...
  if ($resource.type == "server") { // server content
    ...
    sections: [
      { type: "group",
        title: "订阅获取",
        items: [
          { type: "switch", key: "UA", label: "修改 UA",
            onValue: "1", offValue: "" ,
            description: "用小火箭UA下载订阅"}
          ]
      },
      {
        type: "group",
        ...
        items: [
          { type: "text",   key: "regex",  label: "正则保留",
            description: "用正则匹配保留节点", placeholder: "iplc|bgp" },
          { type: "text",   key: "regout", label: "正则删除",
            description: "用正则匹配删除节点", placeholder: "过期" },
          { type: "text",   key: "sort", label: "节点排序",
            description: "1正序 -1倒序 x随机 也可以用>自定顺序", placeholder: "🇭🇰>🇯🇵>🇺🇸" }
        ]
      },
      {
        type: "group",
        title: "参数微调",
        items: [
          { type: "select", key: "emoji", label: "国旗图标",
            description: "快捷加减国旗图标", options: _emojiOptions() },
          { type: "text",   key: "replace", label: "批量替换",
            description: "旧内容@新内容，多组用+连接", placeholder: "香港@HK+台湾@TW" },
          { type: "switch", key: "udp",   label: "UDP转发",
            onValue: "1", offValue: "-1" },
          { type: "switch", key: "tfo",   label: "开启TFO",
            onValue: "1", offValue: "-1" },
          { type: "switch", key: "cert",  label: "证书验证",
            description: "开启安全证书检查", onValue: "1", offValue: "-1" },
          { type: "switch", key: "uot",   label: "UDP隧道",
            description: "仅对SS/SSR节点生效", onValue: "1", offValue: "" },
          { type: "text",   key: "checkurl", label: "测试网址",
            description: "自定延迟测试地址",
      ...
      },
      {
        ...
        title: "节点改名",
        items: [
          { type: "text", key: "delreg",  label: "正则擦除",
            description: "直接删掉名字中匹配到的字串",
            placeholder: "如填 HK 删掉名字里的 HK" },      
          { type: "text", key: "rename",  label: "规则改名",
            description: "格式：旧@新 或 前缀@ 或 @后缀",
            placeholder: "香港@HK+@[1X]" },
          { type: "select", key: "ptn", label: "字母样式",
            description: "英文变成花式字体",
            options: _ptnOptions() },
          { type: "select", key: "npt", label: "数字样式",
            description: "数字变成花式字体",
            options: _nptOptions() }
        ]
      },
      {
        ...
        items: [
          ...
          { type: "switch", key: "info", label: "流量通知",
            onValue: "1", offValue: "", description: "通知推送用量和到期日"},
          { type: "text",   key: "flow", label: "自定流量",
            description: "格式：时间:总GB:已用GB",
            placeholder: "2026-12-31:1000:54" },
          { type: "select", key: "type", label: "强制类型",
            options: [
              { label: "自动判定",     value: ""           },
              { label: "节点 (Nodes)", value: "nodes"      },
              { label: "分流 (Rule)",  value: "rule"       },
              { label: "模块 (Module)",value: "module"     },
              { label: "列表 (List)",  value: "list"       },
              { label: "域名集 (Domain Set)",   value: "domain-set" }
            ] }
        ]
      }
    ]
    ...
    sections: [
      {
        type: "group",
        title: "分流筛选",
        description: "仅对远程分流生效",
        items: [
          { type: "tags", key: "in",  label: "分流保留",
            description: "按关键字保留，换行表示或",
            placeholder: "如：weibo" },
          { type: "tags", key: "out",  label: "分流删除",
            description: "按关键字删除，换行表示或",
            placeholder: "如：weibo" },
          { type: "text", key: "policy", label: "指定策略",
            description: "统一指定到该策略组",
            placeholder: "Shawn" },
          { type: "text", key: "pset",   label: "多组指定",
            description: "正则@策略组，+号连接" },
          { type: "text", key: "regex",   label: "正则保留",
            description: "用正则保留分流" },
          { type: "text", key: "regout",   label: "正则删除",
            description: "用正则删除分流" },
          { type: "text", key: "replace",   label: "正则替换",
            description: "格式：旧正则@新内容" }
        ]
      },
      {
        type : "group",
        title: "节点中转",
        items : [
          { type: "text", key: "via",    label: "中转接口",
             description: "填 0 开启中转分流规则\n" ,
             placeholder: "填写 0" 
          },
          { type: "text",   key: "relay", label: "中转节点",
            description: "关闭首页策略偏好，勿与via同用" ,
            placeholder: "填写中转组名或节点名" },
      ...
      },
      {
        ...
        items: [
          ...
          { type: "switch", key: "cdn",  label: "CDN加速",
            onValue: "1", offValue: "" },
          { type: "select", key: "dst", label: "目标转换",
            options: [
              { label: "默认（不转换）", value: ""        },
              { label: "强制转为重写 (Rewrite)", value: "rewrite" },
              { label: "强制转为分流 (Filter)",  value: "filter"  }
            ] },
          { type: "select", key: "fcr",  label: "物理接口",
            options: [
              { label: "默认",            value: ""  },
              { label: "仅限蜂窝数据",    value: "1" },
              { label: "混合网络接口",    value: "2" },
              { label: "负载均衡模式",    value: "3" }
            ] , description:"强制指定网卡或流量接口" }
        ]
      }
    ]
  };
    ...
    sections: [
      {
        type: "group",
        title: "重写筛选",
        description: "仅对远程重写生效",
        items: [
          { type: "tags", key: "in",  label: "重写保留",
            description: "按关键字保留",
            placeholder: "如：weibo" },
          { type: "tags", key: "out",  label: "重写删除",
            description: "按关键字删除",
            placeholder: "如：weibo" },
          { type: "tags", key: "inhn",  label: "域名保留",
            description: "保留指定域名",
            placeholder: "如：weibo" },
          { type: "tags", key: "outhn", label: "域名删除",
            description: "删除指定域名",
